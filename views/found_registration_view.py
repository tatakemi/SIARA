...
def show_found_registration(page, state, go_to_home_func, show_snack_func):
    ...
    if edit_id is not None:
        try:
            with session_scope() as s:
                current_post = s.query(FoundReport).filter_by(id=edit_id, finder_id=cur["id"]).first()
                if not current_post:
    ...
    title_text = "Relatar Animal Encontrado" if not current_post else "Editar Relato Encontrado"
    button_text = "Relatar" if not current_post else "Atualizar"

    # VARIÁVEIS DE ESTADO DO UPLOAD
    file_path_chosen = None
    file_name_chosen = None
    
    # Campos de formulário (REMOVIDO: required=True)
    species = ft.TextField(label="Espécie/Raça do Animal Encontrado", value=current_post.species if current_post else "")
    location = ft.TextField(label="Local onde o animal foi encontrado", value=current_post.found_location if current_post else "")
    date = ft.TextField(label="Data que o animal foi encontrado (DD/MM/AAAA)", value=current_post.found_date if current_post else "")
    desc = ft.TextField(label="Descrição do animal e circunstâncias do encontro", value=current_post.found_description if current_post else "", multiline=True)
    ...
    def do_register_found(e):
        # Declarações nonlocal no topo (CORREÇÃO DO ERRO ANTERIOR)
        nonlocal current_post, current_image_url, file_path_chosen, file_name_chosen
        
        msg.value = ""
        
        # Validação manual dos campos obrigatórios (mantida)
        ...
        if (current_post and location.value != current_post.found_location) or (not current_post and (lat is None or lon is None)):
        ...
        if file_path_chosen and file_name_chosen:
            ...
            safe_name = f"found_{cur['id']}_{timestamp}{ext}"
        ...
        try:
            with session_scope() as s:
                if current_post:
                    ...
                    current_post.found_location = location.value
                    current_post.found_date = date.value
                    current_post.found_description = desc.value
